...
int setup_client_directory(char *username) {
    ...
    if (env_dir) {
        ...
        snprintf(client_info.sync_dir_path, MAX_PATH_SIZE, "%s/Desktop/sync_dir_%s", home_dir, username);
...
void updateFile(char * nome_arquivo){
    ...
    pthread_mutex_unlock(&mutex3);
    receiveNewFileFromServer(nome_arquivo,diretorio,client_info.server_port,client_info.server_ip);
...
void updateFileServer(char * nome_arquivo){
    ...
    sendNewFileToServer(nome_arquivo,diretorio,client_info.server_port,client_info.server_ip);
...
void removeFileServer(char * nome_arquivo){
    ...
    pthread_mutex_unlock(& mutexCurSen);
    removeFileInServer(nome_arquivo,diretorio,client_info.server_port,client_info.server_ip);
...
void renameFileServer(char * nome_arquivo_novo,char * nome_arquivo_antigo){
    ...
    updateFileName(nome_arquivo_novo,nome_arquivo_antigo,diretorio,client_info.server_port,  client_info.server_ip);
