...
void updateFile(char * nome_arquivo){
    ...
    pthread_mutex_unlock(&mutex3);
    receiveNewFileFromServer(nome_arquivo,client_info.sync_dir_path,client_info.server_port,client_info.server_ip,client_info.username,client_info.client_ip);
...
void updateFileServer(char * nome_arquivo){
    ...
    sendNewFileToServer(nome_arquivo,client_info.sync_dir_path,client_info.server_port,client_info.server_ip,client_info.username,client_info.client_ip);
...
void removeFileServer(char * nome_arquivo){
    ...
    pthread_mutex_unlock(& mutexCurSen);
    removeFileInServer(nome_arquivo,client_info.sync_dir_path,client_info.server_port,client_info.server_ip,client_info.username,client_info.client_ip);
...
void renameFileServer(char * nome_arquivo_novo,char * nome_arquivo_antigo){
    ...
    updateFileName(nome_arquivo_novo,nome_arquivo_antigo,client_info.sync_dir_path,client_info.server_port,  client_info.server_ip,client_info.username,client_info.client_ip);
...
int get_sync_dir() {
    ...
    endereco.sin_port = htons(client_info.server_port);
    inet_pton(AF_INET, client_info.server_ip, &endereco.sin_addr);
    ...
    send(sock, client_info.username, tamanho_nome_cliente, 0);
    int tamanho_nome_IP=strlen(client_info.client_ip);
    ...
    if(codigo_error_device==-1){
        printf("Já existem dois dispositivos conectados, tente novamente mais tarde!");
        return 1;
    ...
    int num_arquivos = receiveFileListFromServer(&arquivosServidor,client_info.sync_dir_path, client_info.server_port, client_info.server_ip,client_info.username,client_info.client_ip);
...
void list_server_files() {
    ...
    int num_arquivos = receiveFileListFromServer(&arquivos_servidor,client_info.sync_dir_path, client_info.server_port, client_info.server_ip,client_info.username,client_info.client_ip);
...
void download_file(const char *filename) {
    ...
    int result = receiveNewFileFromServer((char*)filename, current_dir, client_info.server_port, client_info.server_ip,client_info.username,client_info.client_ip);
...
int main(int argc, char *argv[]) {
    ...
    pthread_cancel(command_thread);  // Garante saída mesmo se travado em fgets()
    ...
    sleep(1);
    if(exitConnectionOnServer(client_info.server_port,client_info.server_ip,client_info.username,client_info.client_ip)!=0)
    printf("Ocorreu um erro ao encerrar\n");
    else{
